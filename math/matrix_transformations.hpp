...
namespace stdmath {
	...
	constexpr mat<T, 3, 3> rotation_matrix(const basic_quaternion<Tq>& q, bool assume_quaternion_is_unit = true) {
	...
	constexpr mat<T, 4, 4> translation_matrix(const vec<T, 3>& position) {
		auto out = mat<T, 4, 4>::identity();
		out[0, 3] = position.x;
		out[1, 3] = position.y;
		out[2, 3] = position.z;
		return out;
	...
	template<typename T = float_t>
	constexpr mat<T, 3, 3> scale_matrix(const vec<T, 3>& scale) {
		auto out = mat<T, 3, 3>::identity();
		out[0, 0] = scale.x;
		out[1, 1] = scale.y;
		out[2, 2] = scale.z;
		return out;
	...
	template<typename T = float_t>
	constexpr mat<T, 4, 4> make_homogenous(const mat<T, 3, 3>& small) {
		return {
	...
	template<typename T = float_t>
	constexpr mat<T, 3, 3> strip_homogenous(const mat<T, 4, 4>& big) {
		return {
	...
	template<typename T = float_t>
	constexpr mat<T, 4, 4> combine_3x3_with_translation(const mat<T, 3, 3>& small, const vec<T, 3> position) {
	...
	template<typename T = float_t, typename Tq = float_t>
	constexpr mat<T, 4, 4> transformation_matrix(const vec<T, 3>& position, const basic_quaternion<Tq>& rotation, const vec<T, 3>& scale) {
		return combine_3x3_with_translation<T>(rotation_matrix<T, Tq>(rotation) * scale_matrix<T>(scale), position);
	}

	template<typename T = float_t>
	constexpr mat<T, 4, 4> look_at_matrix(const vec<T, 3>& observer_position, const vec<T, 3>& target_position, const vec<T, 3>& world_up = {0, 1, 0}) {
		vec<T, 3> forward = normalize(target_position - observer_position);
		vec<T, 3> right = normalize(cross(world_up, forward));
		vec<T, 3> up = cross(forward, right);
	...
	constexpr mat<T, 4, 4> perspective_matrix(
		...
		T far_plane_distance = std::numeric_limits<T>::infinity(),
		bool support_negative_z = true
	) {
		...
		if (std::isinf(far_plane_distance)) {
			...
			if (support_negative_z) {
				...
				out[2, 3] = (static_cast<T>(2) * far_plane_distance * near_plane_distance) / (near_plane_distance - far_plane_distance);
			} else {
				// DirectX/Vulkan style: z in [0, 1]
				out[2, 2] = far_plane_distance / (far_plane_distance - near_plane_distance);
				out[2, 3] = (far_plane_distance * near_plane_distance) / (near_plane_distance - far_plane_distance);
			}
		}
	...
	template<typename T = float_t>
	constexpr mat<T, 4, 4> orthographic_matrix(
		T left, T right,
		T top, T bottom,
		T near_plane_distance = static_cast<T>(.1),
		T far_plane_distance = std::numeric_limits<T>::infinity(),
		...
		mat<T, 4, 4> out = mat<T, 4, 4>::identity();
	...
	template<typename T = float_t>
	constexpr mat<T, 4, 4> pixel_perfect_2d_matrix(
		const vec<T, 2>& screen_size,
		T near_plane_distance = static_cast<T>(.1),
		T far_plane_distance = static_cast<T>(1),
		bool support_negative_z = true
	) {
