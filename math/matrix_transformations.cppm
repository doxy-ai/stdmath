...
namespace stdmath {
	...
	constexpr matrix<T, 4, 4> perspective_matrix(
		...
		T aspect_ratio,
		T near_plane_distance = T(.1),
		T far_plane_distance = T(1000),
		bool support_negative_z = false
	) {
		T f = T(1) / stdmath::tan(fov_y / basic_radian<T>(2));
		...
		// 		out[2, 2] = -T(1);
		// 		out[2, 3] = -(T(2) * near_plane_distance);
		// 	} else {
		...
		// } else {
			...
			if (support_negative_z) {
				...
				out[2, 3] = -(T(2) * far_plane_distance * near_plane_distance) / (far_plane_distance - far_plane_distance);
			} else {
				// DirectX/Vulkan style: z in [0, 1]
	...
	constexpr matrix<T, 4, 4> perspective_matrix(
		...
		return perspective_matrix<T>(fov_y, T(screen_size.x) / screen_size.y, near_plane_distance, far_plane_distance, support_negative_z);
	}
	...
	constexpr matrix<T, 4, 4> orthographic_matrix(
		...
		// } else {
			...
			if (support_negative_z) {
				// OpenGL style: z in [-1, 1]
				out[2, 2] = T(2) / (far_plane_distance - near_plane_distance);
				out[2, 3] = -(far_plane_distance + near_plane_distance) / (far_plane_distance - near_plane_distance);
			} else {
				// DirectX/Vulkan style: z in [0, 1]
				out[2, 2] = T(1) / (far_plane_distance - near_plane_distance);
	...
	constexpr matrix<T, 4, 4> pixel_perfect_2d_matrix(
		const vector<T, 2>& screen_size,
		T near_plane_distance = T(-.1),
		T far_plane_distance = T(1.1),
		bool support_negative_z = false
	) {
		return orthographic_matrix<T>(T(0), screen_size.x, T(0), screen_size.y, near_plane_distance, far_plane_distance, support_negative_z);
	}
