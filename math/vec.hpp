...
namespace stdmath {
	...
	struct vec {
		...
		constexpr vec(T broadcast) { data.fill(broadcast); }
		constexpr vec(std::initializer_list<T> data) : data(data) {}
	...
	struct vec<T, 2> {
		...
		constexpr vec(T broadcast) : x(broadcast), y(broadcast) {}
	...
	struct vec<T, 3> {
		...
		constexpr vec(T broadcast) : x(broadcast), y(broadcast), z(broadcast) {}
	...
	struct vec<T, 4> {
		...
		constexpr vec(T broadcast) : x(broadcast), y(broadcast), z(broadcast), w(broadcast) {}
	...
	template<typename T, size_t N>
	constexpr bool approximately_equal(const vec<T, N>& a, const vec<T, N>& b, stl::float64_t epsilon = std::numeric_limits<stl::float32_t>::epsilon() * 100) {
