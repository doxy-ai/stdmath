...
namespace stdmath {
	...
	struct mat {
		...
		constexpr static mat fill(const T& v = {}) {
			mat out;
			out.data.fill(v);
			return out;
		}
	...
	template<typename T, size_t X, size_t Y>
	constexpr mat<T, Y, X>& transpose(mat<T, Y, X>& out, const mat<T, X, Y>& in) {
	...
	template<typename T, size_t X, size_t Y>
	constexpr mat<T, Y, X> transpose(const mat<T, X, Y>& mat) { stdmath::mat<T, Y, X> out; return transpose(out, mat); }

	// TODO: SIMD
	template<typename T, size_t N>
	constexpr mat<T, N, N> inverse(mat<T, N, N> A) { // Note we need a copy of A so just pass by value
