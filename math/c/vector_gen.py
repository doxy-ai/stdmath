...
def generate_swizzles(name, N, permutations, swig = False):
	...
	if not swig:
		for p in permutations:
			out += f"STDMATH_EXPORT STDMATH_INLINE stdmath_{name}{len(p)} stdmath_{name}{N}_{''.join(p)}(stdmath_{name}{N} v) "
			out += "{ return {v." + ",v.".join(p) + "}; }\n"
			out += f"STDMATH_EXPORT STDMATH_INLINE stdmath_{name}{len(p)} stdmath_{name}{N}_{''.join(p)}_get(stdmath_{name}{N}* p) "
			out += f"{{ return stdmath_{name}{N}_{''.join(p)}(*p); }}\n"

			out += f"STDMATH_EXPORT STDMATH_INLINE stdmath_{name}{N} stdmath_{name}{N}_{''.join(p)}_set(stdmath_{name}{N}* p, stdmath_{name}{len(p)} value) " + "{ "
...
sub = {"int": "int32_t", "uint": "uint32_t", "byte": "uint8_t", "radian": "stdmath_radian", "degree": "stdmath_degree"}
...
for type in slang_gen.scalar_types:
	...
	is_radian = "radian" in type
	is_degree = "degree" in type

	for N in range(1, 5):
		with open(f"gen/{name}{N}.h", "w") as f:
			...
			f.write(
				...
				.replace("{is_degree}", "true" if is_degree else "false")
			)
		with open(f"gen/{name}{N}.i", "w") as f:
			f.write(
