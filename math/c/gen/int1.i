#define STDMATH_EXPORT
#define STDMATH_INLINE
%rename(int1) stdmath_int1;
%include "int1.h"
%include cstring.i

%rename(length_squared) _length_squared;
%rename(length) _length;
%rename(dot) _dot;
%rename(min_element) _min_element;
%rename(max_element) _max_element;
%rename(min) _min;
%rename(max) _max;
%rename(normalize) _normalize;
%rename(lerp) _lerp;
%rename(clamp) _clamp;
%rename(reflect) _reflect;
%rename(approximately_equal) _approximately_equal;
%rename(all_of) _all_of;
%rename(any_of) _any_of;
%rename(none_of) _none_of;
%rename(some_of) _some_of;

%extend stdmath_int1 {
	stdmath_int1(int32_t broadcast) {
		stdmath_int1 *v;
		v = (stdmath_int1 *) malloc(sizeof(stdmath_int1));
		*v = stdmath_int1_broadcast(broadcast);
		return v;
	}

	stdmath_int1(const stdmath_int1& o) {
		stdmath_int1 *v;
		v = (stdmath_int1 *) malloc(sizeof(stdmath_int1));
		memcpy(v, &o, sizeof(o));
		return v;
	}

#if 1 == 4
	stdmath_int1(int32_t x = 0, int32_t y = 0, int32_t z = 0, int32_t w = 0) {
		stdmath_int1 *v;
		v = (stdmath_int1 *) malloc(sizeof(stdmath_int1));
		v->x = x;
		v->y = y;
		v->z = z;
		v->w = w;
		return v;
	}
#elif 1 == 3
	stdmath_int1(int32_t x = 0, int32_t y = 0, int32_t z = 0) {
		stdmath_int1 *v;
		v = (stdmath_int1 *) malloc(sizeof(stdmath_int1));
		v->x = x;
		v->y = y;
		v->z = z;
		return v;
	}
#elif 1 == 2
	stdmath_int1(int32_t x = 0, int32_t y = 0) {
		stdmath_int1 *v;
		v = (stdmath_int1 *) malloc(sizeof(stdmath_int1));
		v->x = x;
		v->y = y;
		return v;
	}
#elif 1 == 1
	stdmath_int1(int32_t x = 0) {
		stdmath_int1 *v;
		v = (stdmath_int1 *) malloc(sizeof(stdmath_int1));
		v->x = x;
		return v;
	}
#endif

#ifndef STDMATH_NO_SWIZZLES

#endif

	stdmath_int1 _add(stdmath_int1 o) {
		return stdmath_int1_add(*self, o);
	}
	stdmath_int1 _subtract(stdmath_int1 o) {
		return stdmath_int1_subtract(*self, o);
	}
	stdmath_int1 _negate() {
		return stdmath_int1_negate(*self);
	}
	stdmath_int1 _multiply(stdmath_int1 o) {
		return stdmath_int1_multiply(*self, o);
	}
	stdmath_int1 _divide(stdmath_int1 o) {
		return stdmath_int1_divide(*self, o);
	}
	stdmath_int1 _scale(int32_t o) {
		return stdmath_int1_scale(*self, o);
	}

#if false
	stdmath_int1 _modulus(stdmath_int1 o) {
		return stdmath_int1_modulus(*self, o);
	}
	stdmath_int1 _logical_not() {
		return stdmath_int1_logical_not(*self);
	}
	stdmath_int1 _bitwise_not() {
		return stdmath_int1_bitwise_not(*self);
	}
	stdmath_int1 _logical_and(stdmath_int1 o) {
		return stdmath_int1_logical_and(*self, o);
	}
	stdmath_int1 _bitwise_and(stdmath_int1 o) {
		return stdmath_int1_bitwise_and(*self, o);
	}
	stdmath_int1 _logical_or(stdmath_int1 o) {
		return stdmath_int1_logical_or(*self, o);
	}
	stdmath_int1 _bitwise_or(stdmath_int1 o) {
		return stdmath_int1_bitwise_or(*self, o);
	}
	stdmath_int1 _bitwise_xor(stdmath_int1 o) {
		return stdmath_int1_bitwise_xor(*self, o);
	}
	stdmath_int1 _shift_left(stdmath_int1 o) {
		return stdmath_int1_shift_left(*self, o);
	}
	stdmath_int1 _shift_right(stdmath_int1 o) {
		return stdmath_int1_shift_right(*self, o);
	}
#endif

	stdmath_bool1 _equal_to(stdmath_int1 o) {
		return stdmath_int1_equal_to(*self, o);
	}
	stdmath_bool1 _not_equal_to(stdmath_int1 o) {
		return stdmath_int1_not_equal_to(*self, o);
	}
	stdmath_bool1 _less_than(stdmath_int1 o) {
		return stdmath_int1_less_than(*self, o);
	}
	stdmath_bool1 _less_than_or_equal_to(stdmath_int1 o) {
		return stdmath_int1_less_than_or_equal_to(*self, o);
	}
	stdmath_bool1 _greater_than(stdmath_int1 o) {
		return stdmath_int1_greater_than(*self, o);
	}
	stdmath_bool1 _greater_than_or_equal_to(stdmath_int1 o) {
		return stdmath_int1_greater_than_or_equal_to(*self, o);
	}

	int32_t _length_squared() {
		return stdmath_int1_length_squared(*self);
	}
	int32_t _length() {
		return stdmath_int1_length(*self);
	}
	int32_t _dot(stdmath_int1 o) {
		return stdmath_int1_dot(*self, o);
	}
	int32_t _min_element() {
		return stdmath_int1_min_element(*self);
	}
	int32_t _max_element() {
		return stdmath_int1_max_element(*self);
	}

	// int32_t elementwise_transform()
	// int32_t reduce_element

	stdmath_int1 _min(stdmath_int1 o) {
		return stdmath_int1_min(*self, o);
	}
	stdmath_int1 _max(stdmath_int1 o) {
		return stdmath_int1_max(*self, o);
	}
	stdmath_int1 _normalize() {
		return stdmath_int1_normalize(*self);
	}
	stdmath_int1 _lerp(stdmath_int1 o, float t) {
		return stdmath_int1_lerp(*self, o, t);
	}
	stdmath_int1 _clamp(stdmath_int1 min, stdmath_int1 max) {
		return stdmath_int1_clamp(*self, min, max);
	}
	stdmath_int1 _reflect(stdmath_int1 n) {
		return stdmath_int1_reflect(*self, n);
	}
	stdmath_bool1 _approximately_equal(stdmath_int1 o) {
		return stdmath_int1_approximately_equal(*self, o);
	}

#if false
	stdmath_bool1 _all_of() {
		return stdmath_bool1_all_of(*self);
	}
	stdmath_bool1 _any_of() {
		return stdmath_bool1_any_of(*self);
	}
	stdmath_bool1 _none_of() {
		return stdmath_bool1_none_of(*self);
	}
	stdmath_bool1 _some_of() {
		return stdmath_bool1_some_of(*self);
	}
#endif

	%cstring_output_allocate_size(char** str, size_t* len, free(*$1))
	void _to_string(const char** str, size_t* len) {
		auto out = stdmath_int1_to_string(*self);
		*str = out.data;
		*len = out.len;
	}

	size_t _size() {
		return 1;
	}

	int32_t _get_element(size_t i) {
		return self->data[i];
	}

	int32_t _set_element(size_t i, int32_t value) {
		return self->data[i] = value;
	}
}