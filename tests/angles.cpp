...
TEST_CASE("angles") {
    ...
    CHECK(std::math::float_equal(std::math::rad2deg<std::float32_t> * std::math::PI, 180.0f, .001));
    CHECK(std::math::float_equal(std::math::deg2rad<std::float32_t> * 180.0f, std::math::PI));
    ...
    CHECK(std::math::float_equal(std::math::cos(r_pi_div_2), 0.0f));
    CHECK(std::math::float_equal(std::math::sin(r_pi_div_2), 1.0f));

    // inverse trig functions
    auto r_asin_05 = std::math::asin(0.5f);
    CHECK(std::math::float_equal(std::math::sin(r_asin_05), 0.5f));

    // atan2
    auto r_atan2_1_1 = std::math::atan2(1.0f, 1.0f);
    CHECK(std::math::float_equal(r_atan2_1_1, 0.785398f));

    // angle_normalize
    // auto d_norm = std::math::angle_wrap<std::math::degree>(540.0); 
    // CHECK(std::math::float_equal(d_norm, 180.0)); // TODO: Why is this not working!?!?

    auto d_norm2 = std::math::angle_wrap<std::math::degree>(-180.0);
    CHECK(std::math::float_equal(d_norm2, 180.0));

    // auto r_norm = std::math::angle_wrap<std::math::radian>(7.0f * std::math::PI);
    // CHECK(std::math::float_equal(r_norm, std::math::PI)); // TODO: Why is this not working!?!?
}