...
TEST_CASE("Quaternion Basics") {
	...
	a = std::math::quaternion::from_axis_angle(std::math::vector<float, 3>{1, 0, 0}, std::math::degree(45));
	CHECK(std::math::approximately_equal(a, std::math::quaternion(.38f, 0.f, 0.f, .92f), .01));
	b = std::math::quaternion::from_axis_angle(std::math::vector<float, 3>{0, 0, 1}, std::math::degree(45));
	...
	CHECK(std::math::approximately_equal(axis, std::math::vector<std::float64_t, 3>(.68, -.28, .68), .01));
	...
	// vectortor rotation
	std::math::vector<std::float64_t, 3> v = {0, 1, 0};
	CHECK(std::math::approximately_equal(a * v, std::math::vector<std::float64_t, 3>(0, .71, .71), .01));
	CHECK(std::math::approximately_equal(b * v, std::math::vector<std::float64_t, 3>(-.71, .71, 0), .01));
	CHECK(std::math::approximately_equal(ab * v, std::math::vector<std::float64_t, 3>(-.71, .5, .5), .01));
}
