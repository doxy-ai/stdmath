...
TEST_CASE("Euler Angles") {
	...
	auto eDegree = eRadian.elementwise_transform([](const std::math::radian& e) -> std::math::degree {
		return e;
	});
	// auto eDegree = eRadian.elementwise_transform();
	CHECK(approximately_equal(eRadian.x, 35.26, .01));
	CHECK(approximately_equal(eRadian.y, -30.00, .01));
	CHECK(approximately_equal(eRadian.z, 35.26, .01));
}
