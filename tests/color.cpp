...
TEST_CASE("color basics") {
	std::math::color32 c1(1.0f, 0.5f, 0.0f, 1.0f);
	CHECK(c1.r == 1.0f);
	CHECK(c1.g == 0.5f);
	CHECK(c1.b == 0.0f);
	CHECK(c1.a == 1.0f);
...
TEST_CASE("color from_hex") {
	...
	CHECK(std::math::approximately_equal(c1.r, 1.0f));
	CHECK(std::math::approximately_equal(c1.g, 128.0f / 255.0f));
	CHECK(std::math::approximately_equal(c1.b, 0.0f));
	CHECK(c1.a == 1.0f);
...
TEST_CASE("color sRGB/Linear conversions") {
	std::math::color32 c(0.5f, 0.5f, 0.5f, 1.0f);
...
TEST_CASE("color HSV conversion") {
	std::math::color32 red(1.0f, 0.0f, 0.0f, 1.0f);
	auto hsv = red.linear_to_hsv();
	CHECK(hsv.x == 0.0f); // Hue
	CHECK(hsv.y == 1.0f); // Saturation
	CHECK(hsv.z == 1.0f); // Value

	auto back_to_red = hsv.hsv_to_linear();
	CHECK(back_to_red.r == 1.0f);
	CHECK(back_to_red.g == 0.0f);
	CHECK(back_to_red.b == 0.0f);
}

TEST_CASE("color luminance and grayscale") {
	std::math::color32 c(1.0f, 1.0f, 0.0f, 1.0f); // Yellow
...
TEST_CASE("color oklab") {
	...
	CHECK(std::math::approximately_equal(back.r, 1.0f));
	CHECK(std::math::approximately_equal(back.g, 0.0f, 1e-5f));
	CHECK(std::math::approximately_equal(back.b, 0.0f, 1e-5f));
}

TEST_CASE("color alpha operations") {
	std::math::color32 c(1.0f, 0.5f, 0.0f, 0.5f);
...
TEST_CASE("color normalization") {
	...
	CHECK(c32.r == 1.0f);
	CHECK(std::math::approximately_equal(c32.g, 127.0f / 255.0f));
